...
(`/wiki-recall`, `/wiki-report`, `/wiki-todo` — `wiki-log`/`wiki-digest`와 같은 계열).
...
`/wiki-log`, `/wiki-recall`, `/wiki-report`, `/wiki-todo`는 어느 프로젝트에서든 호출해야 하므로
...
2. 클론한 디렉터리에서 `make install` 실행 — `~/.config/llm_wiki/repo_path`에 이 clone의 절대경로를
   기록하고, `tooling/commands/{wiki-log,wiki-recall,wiki-report,wiki-todo}.md`를
