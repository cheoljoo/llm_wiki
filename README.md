...
각 커맨드의 호출 예시·상세 사용법은 [GUIDE.md의 "슬래시 커맨드 사용법"](GUIDE.md#3-슬래시-커맨드-사용법) 참고.
...
`/todo-register`, `/todo-complete`는 실행 시마다 `todo/todos.md`를 커밋합니다 (push는 하지 않음).

## 설치 (팀원별 1회)

`/wiki-log`, `/wiki-recall`, `/wiki-report`, `/wiki-todo`, `/wiki-project-done`, `/todo-register`,
`/todo-list`, `/todo-done`, `/todo-all`, `/todo-complete`는 어느 프로젝트에서든 호출해야 하므로
...
수동으로 하고 싶다면:
   ...
   cp tooling/commands/wiki-log.md tooling/commands/wiki-recall.md \
      ...
      tooling/commands/todo-complete.md \
      ~/.claude/commands/
   ```
...
- **`scripts/setup-herdr-vim-nav.sh`** — [herdr](https://herdr.dev)에 [vim-herdr-navigation](https://github.com/paulbkim-dev/vim-herdr-navigation)
  플러그인을 설치하고, Vim split ↔ herdr pane 이동(`prefix+h/j/k/l`)과 workspace/tab 전환
