...
todo/   개인 Todo 관리. log/wiki와는 별개로 /todo-* 커맨드가 읽고 쓴다.
```

- `log/`는 사람이 직접 편집하지 않습니다. `/wiki-log` 명령으로만 추가됩니다.
- `wiki/`는 `/wiki-digest` 명령이 `log/`를 읽어 자동으로 만들고 업데이트합니다.
- `todo/`는 `/todo-register`·`/todo-complete` 명령으로만 갱신됩니다 (자세한 내용은
  [todo/README.md](todo/README.md) 참고).
...
처음 clone했다면 [GETTING_STARTED.md](GETTING_STARTED.md)에서 설치 후 뭘 해보면 되는지 5분 요약부터 보세요.
...
log/wiki와는 별개로, 개인 할 일을 `todo/todos.md`에 등록·추적하는 커맨드 세트입니다
(형식·설계는 [todo/README.md](todo/README.md) 참고).

```
/todo-register 다음 릴리스 전에 herdr 스크립트 문서 업데이트
```
새 Todo를 등록합니다 (`TD-0001`처럼 순번 ID가 자동 부여되고, 등록자·등록시각이 함께 기록됩니다).
...
/todo-all         # 전체
```
각각 상태별로 필터링해 보여줍니다 (읽기 전용). 뒤에 키워드를 붙이면 (`/todo-list herdr`)
내용에 그 키워드가 포함된 항목만 추가로 걸러줍니다. 화면에는 제목만 보여주지만, 매 실행마다
`todo/output/<커맨드명>.md`에 요약 테이블·항목별 상세(메모 포함)까지 덮어써서 남깁니다
(내용이 길어질 수 있어 git에는 커밋하지 않음).

```
/todo-complete TD-0001
```
해당 항목을 완료 처리합니다 (`status`를 `done`으로 바꾸고 완료자·완료시각·완료 경로를 기록).
ID 대신 키워드로도 지정할 수 있으며, 여러 항목이 일치하면 어떤 것인지 되물어봅니다.

```
/todo-memo TD-0003 포트폴리오 초안 방향: ...
```
해당 항목에 메모를 append-only로 추가합니다 (진행 결과, 방문 후 회의록, 나중에 참고할 결론 등).
완료 여부와 무관하게 남길 수 있고, 여러 번 쌓을 수 있습니다.

`/todo-register`, `/todo-complete`, `/todo-memo`는 실행 시마다 `todo/todos.md`를 커밋합니다
(push는 하지 않음).

## 설치 (팀원별 1회)

`/wiki-log`, `/wiki-recall`, `/wiki-report`, `/wiki-todo`, `/wiki-project-done`, `/todo-register`,
`/todo-list`, `/todo-done`, `/todo-all`, `/todo-complete`, `/todo-memo`는 어느 프로젝트에서든 호출해야 하므로
Claude Code의 **user-level 커맨드**로 설치합니다. 개인 clone 경로는 커맨드 파일에 직접 적지 않고,
개인 설정 파일 하나에 저장합니다 — 그래야 `tooling/commands/*.md`가 git으로 업데이트돼도 매번
경로를 다시 적어 넣을 필요가 없습니다.

1. 이 저장소를 원하는 위치에 clone
2. 클론한 디렉터리에서 `make install` 실행 — `~/.config/llm_wiki/repo_path`에 이 clone의 절대경로를
   기록하고, `tooling/commands/{wiki,todo}-*.md`를 `~/.claude/commands/`로 복사합니다.
...
수동으로 하고 싶다면:
   ...
   cp tooling/commands/wiki-log.md tooling/commands/wiki-recall.md \
      ...
      tooling/commands/todo-register.md tooling/commands/todo-list.md \
      tooling/commands/todo-done.md tooling/commands/todo-all.md \
      tooling/commands/todo-complete.md tooling/commands/todo-memo.md \
      ~/.claude/commands/
   ```
