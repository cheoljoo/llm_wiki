...
6. 1번에서 얻은 날짜(`YYYY-MM-DD`)와 내용을 요약한 kebab-case `<slug>`로 파일명을 만든다:
...
8. `<WIKI_REPO_PATH>`에서 `git add log/<새 파일>` 후 `git commit`을 실행한다 (커밋 메시지: `log: <project> - <한 줄 요약>`). **push는 하지 않는다.**
9. 어떤 내용을 기록했는지 사용자에게 한두 문장으로 보고한다.
