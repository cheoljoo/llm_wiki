...
4. 남은 `status: done` 항목들을 `completed_at` 내림차순(최근 완료 순)으로 정렬해 다음 형식으로
   ...
   ```
5. 마지막에 "완료 N건"을 한 줄 덧붙인다. 화면에는 여기까지만 보여준다 (제목 줄 수준의 요약).
6. `mkdir -p <WIKI_REPO_PATH>/todo/output`으로 디렉터리를 만든 뒤,
   `<WIKI_REPO_PATH>/todo/output/todo-done.md`에 지금 보여준 항목들(4단계로 정렬/필터링된 것과
   동일한 목록)로 다음 3개 절을 채워 파일로 저장한다 (매 실행마다 덮어쓴다):
   ```markdown
   # Todo Done — 생성: <date '+%Y-%m-%d %H:%M'>

   ## 1. 제목 목록

   (5번까지 화면에 보여준 것과 동일한 목록 + 마지막 건수 줄)

   ## 2. 요약 테이블

   | ID | 제목 | 등록 | 완료 | 완료 경로 | 내용 요약 | 메모 요약 |
   |---|---|---|---|---|---|---|
   | TD-<번호> | <제목> | <registered_by> <registered_at> | <completed_by> <completed_at> | <completed_via> | <본문 앞부분을 100자 내외로 요약, 넘으면 "..."> | <memo가 있으면 각 memo 첫 줄을 이어붙여 요약, 없으면 "메모 없음"> |

   ## 3. 상세 내용

   ### TD-<번호>: <제목>
   - 등록: <registered_by> <registered_at> / 완료: <completed_by> <completed_at> (<completed_via>)
   ...
   **메모**
   ...
   ```
7. 화면 출력 마지막에 "(상세: todo/output/todo-done.md)"를 한 줄만 덧붙인다 — 화면은 5번까지의
