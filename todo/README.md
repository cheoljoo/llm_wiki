...
## 조회 결과 파일 (`todo/output/`)

`/todo-list`·`/todo-done`·`/todo-all`은 화면에는 지금처럼 제목 줄 수준의 요약만 보여주지만,
매 실행마다 `todo/output/<커맨드명>.md`에 더 자세한 결과를 덮어써서 남긴다 (내용이 길어질 수
있어 git에는 커밋하지 않음 — `.gitignore`에 `todo/output/` 등록됨):

1. 제목 목록 (화면과 동일)
2. 요약 테이블 (ID/제목/마감/등록/완료 등 + 내용 요약 + 메모 요약)
3. 항목별 상세 내용 (본문 전체 + 메모 전체)
