...
### `/todo-memo <ID> <메모 내용>` — Todo 항목에 메모 추가

```
/todo-memo TD-0003 포트폴리오 초안 방향: 예상 고객군은 ...
/todo-memo TD-0002 세무사 방문 회의록: 준비한 2장 확인 완료, 사업성은 ...
```

해당 ID 항목에 메모를 append-only로 추가합니다 — 완료 여부와 무관하게 남길 수 있고, 여러 번
쌓을 수 있습니다 (기존 메모는 지우거나 고치지 않음). 방문 후 회의록, 작업 중 떠오른 생각, 나중에
참고할 결론을 기록해두는 용도입니다. 맨 앞 토큰이 `TD-NNNN` 형식이 아니면 ID를 추측하지 않고
후보를 보여준 뒤 되물어봅니다.
